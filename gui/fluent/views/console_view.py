...
from qfluentwidgets import (
    ...
    FluentIcon, InfoBadge, InfoLevel, InfoBar, InfoBarPosition,
    SegmentedWidget, StrongBodyLabel, CaptionLabel, SubtitleLabel
)
...
class ConsoleView(QWidget):
    ...
    def init_ui(self):
        ...
        # ==========================
        telemetry_card = SimpleCardWidget(self)
        telemetry_layout = QHBoxLayout(telemetry_card)
        telemetry_layout.setContentsMargins(12, 6, 12, 6)
        telemetry_layout.setSpacing(14)

        # 串口状态指示
        self.serial_badge = InfoBadge.attension("串口: 未连接", self)
        telemetry_layout.addWidget(self.serial_badge)

        telemetry_layout.addSpacing(10)

        # 帧率与视觉状态
        self.fps_label = CaptionLabel("FPS: -- | Res: ---", self)
        self.fps_label.setStyleSheet("color: #94a3b8; font-family: monospace;")
        telemetry_layout.addWidget(self.fps_label)

        telemetry_layout.addSpacing(10)

        # 目标相对偏差
        self.target_label = CaptionLabel("Target Δ: [0.0°, 0.0°]", self)
        self.target_label.setStyleSheet("color: #38bdf8; font-family: monospace;")
        telemetry_layout.addWidget(self.target_label)

        telemetry_layout.addStretch()

        # 激光安全状态指示
        self.laser_badge = InfoBadge.success("LASER SAFE", self)
        ...
        mode_title = CaptionLabel("工作模式 / Mode", self)
        ...
        # --- 追踪使能按钮 ---
        self.btn_track = TogglePushButton(FluentIcon.PLAY, "开始追踪 / Track", self)
        ...
        arm_title = CaptionLabel("激光保险 / Arm", self)
        arm_h = QHBoxLayout()
        self.switch_arm = SwitchButton(self)
        ...
        # --- 激光点射按钮 ---
        self.btn_fire = PrimaryPushButton(FluentIcon.MESSAGE, "击发 (Space)", self)
        ...
        self.pwr_label = CaptionLabel("激光功率: 100%", self)
        self.pwr_slider = Slider(Qt.Orientation.Horizontal, self)
        ...
        # --- 一键回中 ---
        self.btn_center = PushButton(FluentIcon.SYNC, "回中 / Center", self)
        ...
        # --- 紧急停止 (E-STOP) ---
        self.btn_estop = PrimaryPushButton(FluentIcon.CANCEL, "🛑 急停 (ESC)", self)
    ...
    def _on_arm_toggled(self, checked: bool):
        ...
        if checked:
            ...
            self.laser_badge.setLevel(InfoLevel.ERROR)
        else:
            self.laser_badge.setText("LASER SAFE")
            self.laser_badge.setLevel(InfoLevel.SUCCESS)
    ...
    def set_laser_firing_visual(self, firing: bool):
        ...
        if firing:
            ...
            self.laser_badge.setLevel(InfoLevel.WARNING)
    ...
    def update_serial_status(self, connected: bool, message: str):
        if connected:
            ...
            self.serial_badge.setLevel(InfoLevel.SUCCESS)
        else:
            self.serial_badge.setText("串口: 未连接")
            self.serial_badge.setLevel(InfoLevel.ATTENTION)
