...
class VisionWorker(QThread):
    ...
    def run(self):
        ...
        while self.is_running:
            ...
            # 简单的帧率控制
            # 由于图像采集(I/O)本身耗时，这里只 sleep 极短时间防止 CPU 100% 空转
            time.sleep(0.005)
    ...
    def process_tracking(self, frame):
        ...
        target_color = getattr(cfg, 'DEFAULT_TRACK_COLOR', 'RED')
        
        # 形态学核 (Kernel)
        kernel = np.ones((5, 5), np.uint8)

        if target_color == 'BLUE':
            # --- 模式 A: 蓝色物体追踪 (Chase Mode) ---
            # 目标: 控制红色激光点 (Red) 去追 蓝色物体 (Blue)
            # 误差 = 蓝色位置 - 红色位置
            ...
            if blue_pos and red_pos:
                # 只有当两个都找到时，才能计算相对误差
                # 我们希望 红色(Laser) 移动到 蓝色(Target) 位置
                # 所以 Error = Target - Laser
                ...
                # 看到目标但没看到激光? 可能是激光跑出画面了，或者被遮挡。
                # 可以在这里写找回逻辑 (Search Logic)，目前暂略。
                if self.frame_count % 60 == 0:
                    print("[VISION] Found Target, but Laser lost.")
            else:
                pass 
            ...
            # --- 模式 B: 红色激光居中 (Default Mode) ---
            # 目标: 将红色激光点保持在画面中心 (Center).
            # 误差 = Center - Red_Pos
            ...
            if contours:
                ...
                if cv2.contourArea(c) > 10:
                    ...
                    # 计算相对于画面中心的误差
                    error_x = int(cfg.CENTER_X - x)
                    error_y = int(cfg.CENTER_Y - y)
